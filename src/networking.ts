...
function printLocalhostUrls(): void {
  ...
  p.log.message('  LLKA-B  http://localhost:8090/_/')
  if (components.includes('llka-verwaltung')) p.log.message('  LLKA-V  http://localhost:3000')
  if (components.includes('llka-resomaker')) p.log.message('  LLKA-R  http://localhost:3001')
}
...
function printManualConfig(domain: string): void {
  ...
  p.log.message('  /api/*  →  localhost:8090   (LLKA-B)')
  p.log.message('  /_/*    →  localhost:8090   (LLKA-B admin)')
  if (components.includes('llka-resomaker')) p.log.message('  /reservierung/*  →  localhost:3001  (LLKA-R)')
  p.log.message('  /*      →  localhost:3000   (LLKA-V)')
  p.log.message('')
